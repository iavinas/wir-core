...
const PAGE = '<!doctype html><title>stage</title>'
  + '<h1>Editor</h1>'
...
test('a control whose only word is its accessible description is findable by that word, prints it, and clicks',
  async () => {
    ...
    try {
      ...
      const full = await session.dispatch({ verb: 'find', name: 'Close Full Screen' }) as Record<string, any>;
      assert.equal(full['population'].matched, 1, `exactly the icon: ${JSON.stringify(full)}`);
      const icon = full['matches'][0];
      assert.equal(icon.role, 'generic', 'the role is the browser\'s own — ARIA forbids naming a generic');
      ...
      assert.ok((close['matches'] as { ref: string }[]).some(m => m.ref === icon.ref),
      ...
      const detail = await session.dispatch({ verb: 'read', target: icon.ref }) as Record<string, any>;
      ...
      const restore = await session.dispatch({ verb: 'find', name: 'restore' }) as Record<string, any>;
      ...
      assert.equal(absent['empty'].normalization,
      ...
      const act = await session.dispatch({ verb: 'act', ref: icon.ref, action: 'click' }) as Record<string, any>;
      assert.equal(act['rejected'], undefined, `the click is accepted: ${JSON.stringify(act['rejected'])}`);
      assert.equal(act['effect'].verdict, 'verified');
      const after = await session.dispatch({ verb: 'find', name: 'windowed' }) as Record<string, any>;
