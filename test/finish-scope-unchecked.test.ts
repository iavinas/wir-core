...
test('unknowable scope is disclosed, and stays distinct from checked-clean',
  async () => {
    ...
    try {
      ...
      const list = [...(session as any).graph.collections]
        .sort((a: any, b: any) => b.itemRefs.length - a.itemRefs.length)[0];
      ...
      assert.equal(before['scopeUnchecked'], undefined,
      ...
      const act = await session.dispatch(
        { verb: 'act', ref: cited, action: 'hover' }) as Record<string, any>;
      ...
      assert.equal(after['scopeUnchecked'], true,
      ...
      const clean = await session.dispatch({ verb: 'finish', answer: 'Host',
        evidenceRefs: [heading] }) as Record<string, any>;
      assert.equal(clean['accepted'], true, JSON.stringify(clean));
      assert.equal(clean['scopeUnchecked'], undefined,
