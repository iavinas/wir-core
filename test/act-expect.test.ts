...
function serve(): Promise<{ server: Server; url: string; close: () => void }> {
  return new Promise((resolve) => {
    const server = createServer((req, res) => {
      ...
      res.end(req.url === '/settings' ? '<!doctype html><title>s</title><h1>Settings page</h1>' : PAGE);
    });
...
const evidence = (r: Record<string, unknown>): string => (r['effect'] as { evidence: string }).evidence;
...
test('expect.sent, .text, .state and .navigation hold and fail with the observation verbatim', async () => {
  ...
  const session = await WirSession.start({ headless: true, expectedAction: 'RETRIEVE', storageStatePath: null });
  try {
    await session.goto(srv.url);
    // A stray key is named, never silently held.
    const stray = await session.dispatch({ verb: 'act', ref: 'n_x', action: 'click',
    ...
    await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'textbox' }), action: 'fill', value: 'Ada' });
    // state: the checkbox, held then failed with the real state.
    const on = await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'checkbox' }),
      action: 'click', expect: { state: { checked: true } } });
    assert.equal(expectation(on).held, true, JSON.stringify(on));
    const off = await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'checkbox' }),
      action: 'click', expect: { state: { checked: true } } });
    assert.equal(expectation(off).held, false, JSON.stringify(off));
    assert.deepEqual(expectation(off).failed, [{ key: 'state.checked', wanted: true, observed: 'checked=false' }]);
    await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'checkbox' }), action: 'click' });

    // sent + text: the AJAX save, declared right.
    const held = await session.dispatch({ verb: 'act', ref: await refOf(session, { name: 'Save' }), action: 'click',
      expect: { sent: { method: 'POST', path: '/api/profile/', fields: { 'user[name]': 'Ada', 'user[notify]': '1' } },
        text: 'Profile saved' } });
    assert.equal(evidence(held), 'request_committed', JSON.stringify(held));
    assert.deepEqual(expectation(held), { declared: { sent: { method: 'POST', path: '/api/profile/',
      fields: { 'user[name]': 'Ada', 'user[notify]': '1' } }, text: 'Profile saved' }, held: true });

    // Declared wrong: the failure carries the receipt's own value and the
    // text's own count. The verdict does not move.
    const wrong = await session.dispatch({ verb: 'act', ref: await refOf(session, { name: 'Save' }), action: 'click',
    ...
    assert.deepEqual(failed.map(f => f.key), ['text', 'sent']);
    assert.match(failed[1]!.observed, /^POST http:\/\/127\.0\.0\.1:\d+\/api\/profile \(answered 200\); fields: user\[name\]=Ada$/);
    assert.equal(failed[0]!.observed, 'present 1 time before dispatch and 1 after — not new to this act');

    // navigation: the served address, right then wrong.
    const nav = await session.dispatch({ verb: 'act', ref: await refOf(session, { name: 'Settings' }), action: 'click',
      expect: { navigation: '/settings/', text: 'Settings page' } });
    assert.equal(expectation(nav).held, true, JSON.stringify(nav));
    await session.goto(srv.url);
    const elsewhere = await session.dispatch({ verb: 'act', ref: await refOf(session, { name: 'Settings' }), action: 'click',
  ...
  } finally { await session.close(); srv.close(); }
});

test('the MUTATE gate: a value_set act is rejected bare and accepted with a held expect.sent', async () => {
  const srv = await serve();
  const session = await WirSession.start({ headless: true, expectedAction: 'MUTATE', storageStatePath: null });
  try {
    await session.goto(srv.url);
    const bare = await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'textbox' }),
      action: 'fill', value: 'Ada' });
    assert.equal(evidence(bare), 'value_set', JSON.stringify(bare));
    const rejected = await session.dispatch({ verb: 'finish', answer: '', evidenceRefs: [bare['actRef'] as string] });
    assert.equal((rejected['rejected'] as { kind: string } | undefined)?.kind, 'finish_rejected',
      `a fill alone proves local state: ${JSON.stringify(rejected)}`);

    const declared = await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'textbox' }),
      action: 'fill', value: 'Grace',
    ...
    const accepted = await session.dispatch({ verb: 'finish', answer: '', evidenceRefs: [declared['actRef'] as string] });
    assert.equal(accepted['accepted'], true, `the declared request was on the wire: ${JSON.stringify(accepted)}`);

    // A held state (or text) alone never admits: local facts stay local.
    const local = await session.dispatch({ verb: 'act', ref: await refOf(session, { role: 'textbox' }),
      action: 'fill', value: 'Linus', expect: { state: { value: 'Linus' } } });
    assert.equal(evidence(local), 'value_set', JSON.stringify(local));
    assert.equal(expectation(local).held, true, JSON.stringify(local));
    assert.equal(session.gateEligibleActs().some(a => a.actRef === local['actRef']), false,
