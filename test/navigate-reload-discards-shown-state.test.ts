...
test('navigate to the served address while a client-side route is shown is refused; force reloads; nothing routed reloads',
  async () => {
    ...
    try {
      ...
      assert.equal(rejected?.kind, 'navigate_would_discard_shown_state',
        `expected the typed refusal, got ${JSON.stringify(refused).slice(0, 400)}`);
      assert.ok(rejected.reason.includes(site.url) && rejected.reason.includes(routed.shownUrl),
