...
const PAGE =
  '<!doctype html><title>unrendered</title>' +
  '<h1>Dashboard</h1>' +
  '<ul>' +
...
test('an empty find discloses unrendered matches under their rendered container, and opening it makes them ordinary matches', async () => {
  ...
  try {
    ...
    const before = (await session.dispatch({ verb: 'find', name: 'review' })) as Record<
      string,
      any
    >;
    ...
    assert.equal(
      c.open,
      JSON.stringify({ verb: 'act', ref: c.ref, action: 'click' }),
    ...
    const asButton = (await session.dispatch({
      verb: 'find',
      name: 'review',
      role: 'button',
    ...
    assert.equal(
      asButton['unrendered'],
      undefined,
      'hidden LINKS are not offered to a button query',
    );

    // 2. A closed <details>: hidden prose with no interactive shape is still a lead.
    const prose = (await session.dispatch({ verb: 'find', name: '30 days' })) as Record<
      string,
      any
    >;
    ...
    assert.equal(
      d.open,
      d.affordances.includes('clickable')
    ...
    assert.equal(
      act['rejected'],
      undefined,
      `the open act is accepted: ${JSON.stringify(act['rejected'])}`,
    );
    ...
    assert.deepEqual(
      names,
      ['All Reviews', 'Pending Reviews'],
      'the hidden links are now real matches',
    );
