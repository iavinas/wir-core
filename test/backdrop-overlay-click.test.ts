...
const PAGE = '<!doctype html><title>backdrop overlay</title>'
  + '<h1>Product</h1>'
...
test('a covered click under a wordless, control-less layer with its own click listener is offered the click on the layer; a wordless layer with no listener is not called a backdrop',
  async () => {
    ...
    try {
      ...
      assert.match(blocked['rejected'].reason, /covered by generic (n_[0-9a-f]+) \(position fixed\)/, 'the layer is in the graph and named by its ref');
      assert.match(blocked['rejected'].reason, /; the covering layer is a backdrop — it has no words and no rendered control, and the layer itself \(div #backdrop\) carries a click listener$/,
        'the class is stated with the two facts it was decided on, and the node that carries the listener');
      assert.match(blocked['rejected'].repair, /^this is a backdrop: it has no words, no controls, and its own click listener; clicking it is how the page dismisses it\. \{"verb":"act","ref":"n_[0-9a-f]+","action":"click"\} to dismiss it, then repeat this act\. Re-reading will NOT help: the page has not changed, something is layered over it\.$/);
      ...
      assert.equal(dismissed['rejected'], undefined, `the offered call is accepted: ${JSON.stringify(dismissed['rejected'])}`);
      assert.equal(dismissed['outcome'], 'delivered');
      assert.equal(await page.evaluate(() => document.getElementById('backdrop')), null, 'the backdrop dismissed itself on the click');
      const again = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const repeated = await session.dispatch({ verb: 'act', ref: again['matches'][0].ref, action: 'click' }) as Record<string, any>;
      assert.equal(repeated['rejected'], undefined, `the previously refused click delivers: ${JSON.stringify(repeated['rejected'])}`);
      ...
      await show('inert');
      const second = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const inert = await session.dispatch({ verb: 'act', ref: second['matches'][0].ref, action: 'click' }) as Record<string, any>;
      assert.equal(inert['rejected']?.kind, 'blocked_by_overlay');
      assert.doesNotMatch(inert['rejected'].reason, /backdrop/, 'a layer nothing listens on is not called a backdrop');
      assert.doesNotMatch(inert['rejected'].reason, /loading indicator/);
      assert.match(inert['rejected'].repair, /^no control inside the overlay carries a dismissal word \(close\/dismiss\/cancel\/exit\/done\/×\/✕\); it cannot take keyboard focus and act has no document-level key, so no key path is offered(; \{"verb":"read","target":"n_[0-9a-f]+"\} lists what the overlay itself contains)?\. Re-reading will NOT help: the page has not changed, something is layered over it\.$/,
