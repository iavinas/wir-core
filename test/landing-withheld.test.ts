...
function serve(): Promise<{ url: string; close: () => void }> {
  return new Promise((resolve) => {
    const s: Server = createServer((q, r) => {
      ...
      r.end(
        '<!doctype html><title>first</title><h1>First</h1>' + '<a href="/second">Go second</a>',
      );
    });
...
test('the landing envelope carries withheld, and the offer is ledgered', async () => {
  ...
  try {
    ...
    assert.ok(
      withheld,
      `withheld must ride the act response: ${JSON.stringify(Object.keys(acted))}`,
    );
    ...
    const offer = session.unconsumedContinuations().find((o) => o.call === withheld.continuation);
    assert.ok(
      offer,
      `the landing's controls page is in the ledger: ` +
