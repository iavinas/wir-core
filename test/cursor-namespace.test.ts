...
test('a cursor no list minted rejects with the computed valid continuation', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      JSON.parse(typo['rejected'].repair),
    ...
    const tgt = (await session.dispatch({ verb: 'read', target: ref, cursor: 'k_25' })) as Record<
      string,
      any
    >;
    assert.equal(tgt['rejected']?.kind, 'invalid_args', JSON.stringify(tgt).slice(0, 300));
    assert.equal(
      JSON.parse(tgt['rejected'].repair).target,
      ref,
    ...
    const cont = (await session.dispatch(JSON.parse(bare['withheld'].continuation))) as Record<
      string,
      any
    >;
    assert.equal(cont['rejected'], undefined, JSON.stringify(cont['rejected']));
    assert.equal(
      (cont['controls'] as unknown[]).length,
      10,
