...
test('a finish citing the document root ref is accepted', async () => {
  const session = await startOn(
    '<!doctype html><title>root</title><h1>Heading</h1><p>Body text</p>',
  );
  try {
...
test('a state-only find reaches the verb instead of being rejected', async () => {
  ...
  try {
    ...
    assert.ok(
      !('rejected' in found),
      `state-only find must not be rejected: ${JSON.stringify(found)}`,
    );
