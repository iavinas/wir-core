...
const PAGE = '<!doctype html><title>transient overlay</title>'
...
test('a covered click under a loading indicator is offered the literal act-with-until, and that call waits for the indicator to leave; a layer with an exit keeps the exit',
  async () => {
    ...
    try {
      ...
      const show = (id: string): Promise<void> => page.evaluate((i) => { document.getElementById(i)!.hidden = false; }, id);
      const removeIn = (id: string, ms: number): Promise<void> =>
        page.evaluate(([i, t]) => { setTimeout(() => document.getElementById(i as string)!.remove(), t as number); }, [id, ms]);
      const behind = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const ref = behind['matches'][0].ref as string;

      // 1. The words rule: `<img alt="Loading…">` — gone is offered, on the image's own words.
      await show('imgmask');
      const words = await session.dispatch({ verb: 'act', ref, action: 'click' }) as Record<string, any>;
      assert.equal(words['rejected']?.kind, 'blocked_by_overlay');
      assert.match(words['rejected'].reason, /covered by div #imgmask \(position fixed/, 'the layer is still named');
      assert.match(words['rejected'].reason, /the covering layer is a loading indicator — words: <img alt> "Loading…" contains "loading"/,
        'the rule that fired is stated, in the words it was decided on');
      assert.match(words['rejected'].repair, /^this overlay is a loading indicator; the page will change on its own: repeat the act with until, or read after it settles\. /);
      assert.doesNotMatch(words['rejected'].repair, /Re-reading will NOT help/, 'the sentence that was wrong for this class is gone');
      ...
      assert.deepEqual(call, { verb: 'act', ref, action: 'click', until: { gone: 'Loading…' } },
      ...
      const waited = await session.dispatch(JSON.parse(offered[1] as string)) as Record<string, any>;
      assert.equal(waited['rejected'], undefined, `the offered call is accepted: ${JSON.stringify(waited['rejected'])}`);
      assert.equal(waited['outcome'], 'delivered');
      assert.equal(waited['effect'].until?.verdict, 'condition_met', JSON.stringify(waited['effect'].until));
      assert.match(waited['effect'].until.observed, /^the covering loading indicator left the target after \d+ ms of waiting before dispatch; no rendered node carries "Loading…"/,
      ...
      await show('spinmask');
      const again = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const ref2 = again['matches'][0].ref as string;
      const spin = await session.dispatch({ verb: 'act', ref: ref2, action: 'click' }) as Record<string, any>;
      assert.equal(spin['rejected']?.kind, 'blocked_by_overlay');
      assert.match(spin['rejected'].reason, /the covering layer is a loading indicator — animated-only: it has no words and its only content is 3 animated elements \(animation-name "fade"\)/);
      const offered2 = waitCall.exec(spin['rejected'].repair);
      assert.ok(offered2);
      const call2 = JSON.parse(offered2[1] as string);
      assert.deepEqual(call2, { verb: 'act', ref: ref2, action: 'click', until: { network: 'idle' } },
        'with no word to watch leave, the wait is on the wire');
      await removeIn('spinmask', 700);
      const waited2 = await session.dispatch(JSON.parse(offered2[1] as string)) as Record<string, any>;
      assert.equal(waited2['rejected'], undefined, JSON.stringify(waited2['rejected']));
      assert.equal(waited2['effect'].until?.verdict, 'condition_met', JSON.stringify(waited2['effect'].until));
      assert.match(waited2['effect'].until.observed, /^the covering loading indicator left the target after \d+ ms of waiting before dispatch; /);

      // 3. CONTROL: a loading word beside a Close button — the exit is listed, the text is unchanged.
      await page.goto(`file://${join(dir, 'a.html')}`);
      await show('closable');
      const third = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const closable = await session.dispatch({ verb: 'act', ref: third['matches'][0].ref, action: 'click' }) as Record<string, any>;
      assert.equal(closable['rejected']?.kind, 'blocked_by_overlay');
      assert.doesNotMatch(closable['rejected'].reason, /loading indicator/, 'a layer with its own exit is not called transient');
      assert.match(closable['rejected'].repair, /^the overlay's own controls whose words contain one of close\/dismiss\/cancel\/exit\/done\/×\/✕: \[\{"ref":"[^"]+","role":"button","name":"Close"\}\]\. \{"verb":"act","ref":"[^"]+","action":"click"\} to dismiss it, then repeat this act\. Re-reading will NOT help: the page has not changed, something is layered over it\.$/);
      ...
      await page.goto(`file://${join(dir, 'a.html')}`);
      const free = await session.dispatch({ verb: 'find', name: 'Show / Hide Editor' }) as Record<string, any>;
      const ok = await session.dispatch({ verb: 'act', ref: free['matches'][0].ref, action: 'click' }) as Record<string, any>;
