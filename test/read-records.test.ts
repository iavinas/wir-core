...
const PAGE = '<!doctype html><title>records</title><h1>Catalog</h1>'
  ...
  + '<section aria-label="Wide">' + Array.from({ length: 45 }, (_, i) => `<p>entry ${i}</p>`).join('') + '</section>';

function serve(): Promise<{ url: string; close: () => void }> {
  return new Promise(resolve => {
    const s: Server = createServer((_q, r) => { r.writeHead(200, { 'content-type': 'text/html' }); r.end(PAGE); });
...
const strip = (r: Record<string, unknown>): Record<string, unknown> => {
  const { links: _l, values: _v, numbers: _n, ...rest } = r; return rest;
};
...
test('rows carry numbers and values by default, links with fields:true; nothing shown today is removed', async () => {
  ...
  try {
    ...
    const deals = (overview['collections'] as any[]).find(c => c.itemCount === 2 && String(c.items?.[0]?.content ?? '').includes('Deal'));
    assert.ok(grid && deals, `precondition: the table body (3) and the list (2) compile as collections: ${JSON.stringify(overview['collections'])}`);

    // ---- the table, default arm --------------------------------------------
    const t = await session.dispatch({ verb: 'read', target: grid.ref, all: true }) as Record<string, any>;
    ...
    for (const r of rows) { assert.equal(typeof r.ref, 'string'); assert.equal(typeof r.label, 'string'); }
    ...
    assert.deepEqual(rows[0]!.numbers, [
      { raw: '87%', number: 87, unit: '%' }, { raw: '12', number: 12 },
    ...
    assert.deepEqual(v0.map(v => [v.role, v.name, v.value]),
      [['spinbutton', 'Qty', '2'], ['combobox', 'Ship', 'Express']]);
    for (const v of v0) assert.equal(typeof v.ref, 'string');
    assert.deepEqual((rows[1]!.values as any[]).map(v => v.value), ['1', 'Standard']);
    ...
    assert.deepEqual(t['table'].record.moreFields,
      { fields: ['links'], continuation: `{"verb":"read","target":"${grid.ref}","all":true,"fields":true}` });
    // The byte bound the change was allowed: a record never doubles its row.
    assert.ok(bytes(rows) <= 2 * bytes(rows.map(strip)),
      `record ${bytes(rows)} B vs text ${bytes(rows.map(strip))} B`);

    // ---- the table, fields:true --------------------------------------------
    const w = await session.dispatch(JSON.parse(t['table'].record.moreFields.continuation)) as Record<string, any>;
    ...
    assert.deepEqual((wide[0]!.links as any[]).map(l => [l.name, l.href]), [
    ...
    assert.deepEqual(wide.map(r => [r.ref, r.numbers, r.values]), rows.map(r => [r.ref, r.numbers, r.values]));

    // ---- the list: numbers with trailing units, links ------------------------
    const d = await session.dispatch({ verb: 'read', target: deals.ref, all: true, fields: true }) as Record<string, any>;
    const items: Record<string, any>[] = d['table'].rows;
    assert.deepEqual(items[0]!.numbers, [{ raw: '18W', number: 18, unit: 'W' }, { raw: '$11.83', number: 11.83, unit: '$' }]);
    assert.deepEqual(items[1]!.numbers, [{ raw: '6', number: 6 }, { raw: '$9.99', number: 9.99, unit: '$' }]);
    assert.deepEqual((items[1]!.links as any[]).map(l => [l.name, l.href]), [['Deal two', `${srv.url}/d/2`]]);

    // ---- steering: fields without all is a rejection, not a silent no-op ----
    const bare = await session.dispatch({ verb: 'read', target: grid.ref, fields: true } as any) as Record<string, any>;
    assert.equal(bare['rejected']?.kind, 'invalid_args', JSON.stringify(bare));
    assert.equal(bare['rejected'].repair, `{"verb":"read","target":"${grid.ref}","all":true,"fields":true}`);
...
test('a find with exactly one match carries its read inline as detail; two or more do not', async () => {
  ...
  try {
    ...
    const one = await session.dispatch({ verb: 'find', role: 'link', name: 'My Account' }) as Record<string, any>;
    ...
    assert.deepEqual(one['detail'], { node: plain['node'], children: plain['children'], childrenTotal: plain['childrenTotal'] },
      'detail is the read, verbatim');
    assert.equal(one['detail'].node.ref, ref);
    // Two or more: unchanged.
    const many = await session.dispatch({ verb: 'find', role: 'link', name: 'Deal' }) as Record<string, any>;
    ...
    // with no cursorReset.
    const wide = await session.dispatch({ verb: 'find', role: 'region', name: 'Wide' }) as Record<string, any>;
    ...
    const rest = await session.dispatch(JSON.parse(det.withheld.continuation)) as Record<string, any>;
