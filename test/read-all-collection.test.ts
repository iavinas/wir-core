...
const PAGE =
  '<!doctype html><title>t</title><h1>Orders</h1>' +
  '<button id="prune">Prune</button>' +
  '<ul id="grid">' +
  Array.from(
    { length: ROWS },
    (_, i) =>
      `<li id="r${i}"><span>row-${i}-of-${ROWS}</span> <span>$${(i + 1) * 1.5}</span> <span>${filler}</span></li>`,
  ).join('') +
  '</ul>' +
...
test('all:true states the population first, pages by bytes at row boundaries, and delivers every row once', async () => {
  ...
  try {
    ...
    const p1 = (await session.dispatch({ verb: 'read', target: col.ref, all: true })) as Record<
      string,
      any
    >;
    ...
    const now = (await session.dispatch({ verb: 'read', target: col.ref, all: true })) as Record<
      string,
      any
    >;
    ...
    while (cur) {
      ...
      cur = cur['table'].moreRows
        ? ((await session.dispatch(JSON.parse(cur['table'].moreRows.continuation))) as Record<
            string,
            any
          >)
        : null;
    ...
    assert.deepEqual(
      skipped,
      [],
      'no row on the page may be undelivered after the chain is consumed',
    );
    ...
    const r = (await session.dispatch({ verb: 'read', target: item, all: true })) as Record<
      string,
      any
    >;
