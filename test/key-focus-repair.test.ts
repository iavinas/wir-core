...
const PAGE = '<!doctype html><title>focus</title><h1>Focus</h1>'
...
test('a key on a node that cannot focus is delivered to the ancestor that can',
  async () => {
    ...
    await withPage(async (s) => {
      ...
      const r = await s.dispatch(
        { verb: 'act', ref, action: 'key', value: 'ArrowRight' }) as Record<string, any>;
      ...
      assert.match(after, /cannot take focus/,
        `the delta says the target could not take it: ${after}`);
      assert.match(after, /nearest focusable ancestor/,
        `and names where it went instead: ${after}`);
      ...
      assert.deepEqual(keys, ['ArrowRight'],
        'and the element that listens actually received it');
    });
  });

test('CONTROL — a node with no focusable ancestor says so, inventing nothing',
  async () => {
    await withPage(async (s) => {
      ...
      assert.match(repair, /no ancestor/,
        `it states the absence rather than naming a target: ${repair}`);
      assert.doesNotMatch(repair, /"verb":"act"/,
