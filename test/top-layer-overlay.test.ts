...
const PAGE = '<!doctype html><title>top layer</title>'
  + '<h1>Product</h1>'
...
test('a covered click names the overlay root and lists its own dismiss controls; a top-layer element is a region in read',
  async () => {
    ...
    try {
      ...
      const blocked = await session.dispatch({ verb: 'act', ref, action: 'click' }) as Record<string, any>;
      ...
      assert.match(reason, /received by div "Elements" instead/, 'the element under the pointer is still named');
      assert.match(reason, /covered by div #stage "Elements" \(position fixed/,
        'the LAYER is named — the fixed stage, not the absolute panel or the heading');
      assert.match(repair, /"description":"Close Full Screen"/, 'the exit is listed by its description');
      assert.match(repair, /Re-reading will NOT help: the page has not changed, something is layered over it\./);
      const cont = overlayRepair.exec(repair);
      assert.ok(cont, 'a literal act click continuation is offered');
      const exit = await session.dispatch({ verb: 'find', name: 'Close Full Screen' }) as Record<string, any>;
      assert.equal(JSON.parse(cont[1] as string).ref, exit['matches'][0].ref, 'and it is the exit control itself');
      ...
      assert.equal(lifted['rejected'], undefined, `the continuation is accepted: ${JSON.stringify(lifted['rejected'])}`);
      const again = await session.dispatch({ verb: 'act', ref, action: 'click' }) as Record<string, any>;
      ...
      await page.evaluate(() => { document.getElementById('stage')!.remove(); document.getElementById('bare')!.hidden = false; });
      const link = await session.dispatch({ verb: 'find', name: 'Uncovered link' }) as Record<string, any>;
      const bare = await session.dispatch({ verb: 'act', ref: link['matches'][0].ref, action: 'click' }) as Record<string, any>;
      ...
      assert.doesNotMatch(bare['rejected'].reason, /loading indicator/, 'no rule fires for a bare layer with neutral words');
      assert.match(bare['rejected'].repair, /^no control inside the overlay carries a dismissal word/);
      assert.match(bare['rejected'].repair, /cannot take keyboard focus/, 'no key path is invented for an unfocusable root');
      ...
      await page.goto(`file://${join(dir, 'a.html')}`);
      await page.evaluate(() => { document.getElementById('stage')!.remove(); (document.getElementById('dlg') as HTMLDialogElement).showModal(); });
      ...
      const under = await session.dispatch({ verb: 'find', name: 'Uncovered link' }) as Record<string, any>;
      const covered = await session.dispatch({ verb: 'act', ref: under['matches'][0].ref, action: 'click' }) as Record<string, any>;
      assert.equal(covered['rejected']?.kind, 'blocked_by_overlay');
      assert.match(covered['rejected'].reason, new RegExp(`covered by dialog ${dlg.ref} \\(in the browser's top layer\\)`),
        'the dialog is named by its ref, as a top-layer root');
      assert.match(covered['rejected'].repair, /\(or that submit a form method=dialog\): \[\{"ref":"[^"]+","role":"button","name":"OK"\}\]/,
      ...
      assert.equal((after['regions'] as { topLayer?: boolean }[]).some(r => r.topLayer), false,
        'the closed dialog leaves the top layer, and the overview');

      // 4. CONTROL: with no layer, an uncovered click is simply delivered.
      const free = await session.dispatch({ verb: 'find', name: 'Uncovered link' }) as Record<string, any>;
