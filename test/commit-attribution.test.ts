...
function serve(): Promise<{ server: Server; base: string }> {
  const server = createServer((req, res) => {
    if (req.method === 'POST' && req.url === '/slow') {
      ...
      setTimeout(() => { res.writeHead(302, { location: '/done' }); res.end(); }, SERVER_THINK_MS);
      return;
    }
    if (req.method === 'POST' && req.url === '/refuse') {
      res.writeHead(422, { 'content-type': 'text/html' });
      res.end('<!doctype html><title>rejected</title><h1>The change you requested was rejected</h1>');
      return;
    }
    const body = req.url === '/'
      ? PAGE
