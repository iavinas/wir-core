...
const refsIn = (v: unknown, into: Set<string>): void => {
  ...
  if (Array.isArray(v)) { for (const x of v) refsIn(x, into); return; }
...
test('bound-label offers are minted only for items that rode the payload', async () => {
  ...
  try {
    ...
    const found = await session.dispatch({ verb: 'find', name: 'needle-bearing' }) as Record<string, any>;
    ...
    assert.deepEqual(phantoms.map(p => p.call), [],
    ...
    const itemRef = (found['matches'] as any[])
      .map(m => m['item']?.ref).find(r => typeof r === 'string');
    assert.ok(itemRef, `precondition: the match rides inside an item: ${JSON.stringify(found['matches'])}`);
    assert.ok(offers.some(o => o.call.includes(itemRef)),
