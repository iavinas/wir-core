...
const PAGE =
  '<!doctype html><title>attribution</title>' +
  '<h1>Store</h1>' +
...
test('an empty find names the filter that emptied it, and offers the call that drops it', async () => {
  ...
  try {
    ...
    assert.equal(
      empty.meaning,
      'no node in scope satisfied every filter at once',
