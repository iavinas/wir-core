...
const PAGE =
  '<!doctype html><title>state</title><h1>State</h1>' +
...
const byName = async (s: WirSession, re: RegExp): Promise<any> => {
  ...
  assert.ok(
    c,
    `precondition: ${re} compiled: ${JSON.stringify((ov['controls'] ?? []).map((x: any) => x.name))}`,
  );
  return c;
};
...
test('ticking a checkbox is verified, and the delta says what changed', async () => {
  await withPage(async (s) => {
    ...
    const r = (await s.dispatch({ verb: 'act', ref: cb.ref, action: 'click' })) as Record<
      string,
      any
    >;
    assert.equal(
...
test('a toggle button flipping aria-pressed is verified too', async () => {
  ...
  await withPage(async (s) => {
    ...
    const r = (await s.dispatch({ verb: 'act', ref: b.ref, action: 'click' })) as Record<
      string,
      any
    >;
...
test('a tri-state box going mixed -> true is a change, not a flattened false', async () => {
...
test('CONTROL — a click that changes NOTHING is still not verified', async () => {
  ...
  await withPage(async (s) => {
    const r0 = (await s.dispatch({ verb: 'find', name: 'nothing happens here' })) as Record<
      string,
      any
    >;
    const m = (r0['matches'] ?? [])[0];
    assert.ok(m, 'precondition: the inert paragraph is findable');
    const r = (await s.dispatch({ verb: 'act', ref: m.ref, action: 'click' })) as Record<
      string,
      any
    >;
    assert.notEqual(
