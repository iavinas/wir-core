...
test('navigate to the shown address of a client-side route is refused, and force loads it', async () => {
  ...
  try {
    ...
    assert.ok(
      rejected.reason.includes(site.url) && rejected.reason.includes(shown),
...
test('a real link click moves the served document, and navigating there again is not refused', async () => {
  const site = await serve();
  const session = await WirSession.start({
    headless: true,
    expectedAction: 'NAVIGATE',
...
test('a composed query address is refused on a client-side route, admitted and disclosed elsewhere', async () => {
  ...
  try {
    ...
    assert.equal(
      (refused['rejected'] as { kind: string } | undefined)?.kind,
    ...
    assert.equal(rep.addressShown, false);
    ...
    const admitted = await session.dispatch({
      verb: 'navigate',
      url: `${url}directions?engine=car&route=5,5;6,6`,
    });
    ...
    assert.equal(rep2.addressShown, false);
  } finally {
    await session.close();
    close();
  }
