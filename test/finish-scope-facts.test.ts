...
const PAGE = '<!doctype html><title>scope</title><h1>Nested</h1>'
  + `<ul><li><ul>${rows}</ul></li><li>outer tail</li></ul>`;

test('the finish gate names the innermost population, and stays silent otherwise',
  async () => {
    ...
    try {
      ...
      const cols = [...(session as any).graph.collections]
        .sort((a: any, b: any) => b.itemRefs.length - a.itemRefs.length);
      const inner = cols[0];
      assert.ok(inner.itemRefs.length > 10,
      ...
      const fin = await session.dispatch(
        { verb: 'finish', answer: 'inner row 1', evidenceRefs: [cited] }) as Record<string, any>;
      assert.equal(fin['accepted'], true, JSON.stringify(fin));

      const scope = fin['scope'];
      assert.ok(Array.isArray(scope) && scope.length === 1, `expected one fact: ${JSON.stringify(fin)}`);
      assert.equal(scope[0].collection, inner.ref,
      ...
      assert.ok(scope[0].delivered < scope[0].items,
        'and how much of it was actually handed over');
      assert.equal(scope[0].continuation, JSON.stringify({ verb: 'read', target: inner.ref }),
      ...
      const quiet = await session.dispatch(
        { verb: 'finish', answer: 'Nested', evidenceRefs: [heading] }) as Record<string, any>;
      assert.equal(quiet['accepted'], true, JSON.stringify(quiet));
      assert.equal(quiet['scope'], undefined,
