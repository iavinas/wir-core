...
function serve(): Promise<{ url: string; close: () => void }> {
  return new Promise(resolve => {
    const s: Server = createServer((q, r) => {
      ...
      r.end(q.url?.startsWith('/search') ? '<!doctype html><title>s</title><h1>Results</h1>' : PAGE);
    });
...
test('an unambiguous alias is applied and echoed; an ambiguous one still rejects', async () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(cont), { verb: 'read', cursor: 'c_50' }, 'precondition: the overview mints c_50');
    ...
    for (const cursor of [cont, JSON.parse(cont)]) {
      ...
      assert.equal((r['controls'] as unknown[]).length, 11, 'c_50 reached the withheld tail: 60 links + the search field - 50');
    }
    ...
    const foreign = await session.dispatch(
      { verb: 'read', cursor: '{"verb":"find","name":"link","cursor":"c_20"}' }) as Record<string, any>;
    ...
    for (const key of ['query', 'text']) {
      const r = await session.dispatch({ verb: 'find', [key]: 'link 3' } as any) as Record<string, any>;
      assert.equal(r['rejected'], undefined, JSON.stringify(r['rejected']));
    ...
    const both = await session.dispatch({ verb: 'find', name: 'link 3', query: 'link 4' } as any) as Record<string, any>;
    ...
    const box = (await session.dispatch({ verb: 'find', role: 'textbox', name: 'Search' }) as Record<string, any>)['matches'][0];
    const fill = await session.dispatch(
      { verb: 'act', target: box.ref, action: 'fill', value: 'squash' } as any) as Record<string, any>;
    assert.equal(fill['rejected'], undefined, JSON.stringify(fill['rejected']));
    assert.deepEqual(fill['applied'], { verb: 'act', ref: box.ref, action: 'fill', value: 'squash' });
    ...
    const ret = await session.dispatch(
      { verb: 'act', ref: box.ref, action: 'key', value: 'Return' }) as Record<string, any>;
    ...
    assert.match(session.host.page.url(), /\/search\?q=squash$/, 'Enter reached the page and submitted the form');
    ...
    const h1 = (await session.dispatch({ verb: 'find', role: 'heading', name: 'Results' }) as Record<string, any>)['matches'][0];
    const chord = await session.dispatch(
      { verb: 'act', ref: h1.ref, action: 'key', value: 'Control+Down' }) as Record<string, any>;
    ...
    const plain = await session.dispatch(
      { verb: 'act', ref: h1.ref, action: 'key', value: 'ArrowDown' }) as Record<string, any>;
