      ...
      if (q.url === '/second') {
        r.end(
          '<!doctype html><title>second</title><h1>Second</h1>' +
            '<ul>' +
            Array.from({ length: 6 }, (_, i) => `<li>landed row ${i}</li>`).join('') +
            '</ul>',
        );
        return;
      }
...
test('a document-replacing act carries a landing overview whose refs are citable', async () => {
  ...
  try {
    ...
    const link = (ov['controls'] ?? []).find((c: any) =>
    ...
    assert.ok(
      landing,
      `a replacing act must carry its landing: ${JSON.stringify(Object.keys(acted))}`,
    );
    ...
    const walk = (v: any): void => {
      if (typeof v === 'string') {
        if (/^n_[0-9a-f]{12,40}$/.test(v)) refs.push(v);
        return;
      }
      if (Array.isArray(v)) {
        v.forEach(walk);
...
test('a same-document act carries no landing, and the episode is unharmed', async () => {
  ...
  try {
    ...
    const after = (await session.dispatch({ verb: 'read', target: button.ref })) as Record<
      string,
      any
    >;
    assert.equal(
