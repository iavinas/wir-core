...
const openPage = async () => {
  ...
  const refs = [...new Set(
    [...JSON.stringify(overview).matchAll(/n_[0-9a-f]{12,40}/g)].map(m => m[0]))];
  const observed = refs[0];
  assert.ok(observed, 'precondition: the overview delivered a ref to cite');
  return { session, observed };
};

test('two bad finishes alternating are each told they have been seen before',
  async () => {
    ...
    try {
      ...
      assert.ok(a2.reason.startsWith(a1.reason),
...
test('an accepted finish gains nothing from the ledger — same verdict, same bytes',
  async () => {
    const { session, observed } = await openPage();
    try {
      ...
      assert.equal(stable(d2), stable(d1),
      ...
      assert.equal(stable(d3), stable(d1),
