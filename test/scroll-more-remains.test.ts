...
const PAGE = '<!doctype html><title>scroll</title><h1>Scroll</h1>'
...
const inner = async (s: WirSession): Promise<string> => {
  ...
  const m = (r['matches'] ?? [])[0];
  assert.ok(m, `precondition: a paragraph inside the box is findable: ${JSON.stringify(r['matches'])}`);
  return m.ref;
};
...
test('a scroll with more below reports `scrolled`, even when the DOM is unchanged',
  async () => {
    await withPage(async (s) => {
      ...
      assert.equal(r['effect']?.evidence, 'scrolled',
        'more remains below, so this is new content to the reader — not a stop signal');
      assert.equal(r['effect']?.verdict, 'verified');
    });
  });

test('CONTROL — at the end it STILL says scrolled_no_new_content, so stop arrives',
  async () => {
    await withPage(async (s) => {
      const ref = await inner(s);
      ...
      const r = await s.dispatch({ verb: 'act', ref, action: 'scroll' }) as Record<string, any>;
      assert.equal(r['effect']?.evidence, 'scrolled_no_new_content',
        'the stop signal must still exist, or a caller scrolls forever');
    });
  });

test('scroll "end" reaches the bottom in ONE call, and says how far it went',
  async () => {
    ...
    await withPage(async (s) => {
      ...
      const r = await s.dispatch(
        { verb: 'act', ref, action: 'scroll', value: 'end' }) as Record<string, any>;
      ...
      assert.match(String(r['effect']?.delta?.after), /\[\d+ screens?\]/,
...
test('CONTROL — an unknown scroll value is refused, never guessed at', async () => {
  ...
  await withPage(async (s) => {
    const ref = await inner(s);
    const r = await s.dispatch(
      { verb: 'act', ref, action: 'scroll', value: 'sideways' }) as Record<string, any>;
    assert.equal(r['rejected']?.kind, 'invalid_args');
    assert.match(String(r['rejected']?.repair), /"value":"end"/,
