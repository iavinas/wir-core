...
test('a control two levels down arrives as a ref, not as a count', async () => {
  ...
  try {
    ...
    const carrier = kids.find((k) =>
      (k.descendants?.reachable ?? []).some((x) => x.ref === textbox.ref),
    );
    assert.ok(
      carrier,
    ...
    assert.ok(
      reachable.some((r) => r.ref === textbox.ref && r.name === 'Username or email address'),
    ...
    assert.ok(
      reachable.some((r) => r.ref === heading.ref),
...
test('the per-response bound counts exactly what it drops, and its continuation is a real call', async () => {
  ...
  try {
    ...
    for (const k of withDesc) {
      ...
      if (left > 0)
        assert.equal(d.moreReachable!.estimated, false, 'the residual is counted, never estimated');
    }
    ...
    const reached =
      ((resumed['children'] ?? []) as Child[]).flatMap((c) => c.descendants?.reachable ?? [])
        .length +
